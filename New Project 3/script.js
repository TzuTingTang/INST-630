...
let reverb         = null;
let filterNode     = null;
let tremoloNode    = null;   // Integrity → tremolo (amplitude wobble)
let pannerNode     = null;   // Freedom   → stereo panning
let volNode        = null;
let sequence       = null;
let audioReady     = false;

// ────── GeoJSON country name → WHR country name ──────
const GEO_TO_WHR = {
  "USA":                             "United States",
...
async function loadData() {
  ...
  // Auto-select first country so Play works immediately
  if (data.length > 0) {
    const first = data[0];
    document.getElementById("countrySelect").value = first.country;
    renderBars(first);
  } else {
    renderBars(null);
  }
...
function happinessColor(norm) {
  ...
  // Terracotta(0) → warm sand(0.5) → sage green(1)
  let r, g, b;
  if (norm <= 0.5) {
    const t = norm * 2;
...
// ────── Select country (shared entry point for map click + dropdown) ──────
function selectCountry(countryName) {
  const d = countryByName(countryName);
  if (!d) return;

  document.getElementById("countrySelect").value = countryName;
  highlightOnMap(countryName);
  renderBars(d);
  updateRadarChart(d);
...
function updateRadarChart(d) {
  // Color matches choropleth: low happiness → terracotta, high → sage green
  const n = safe(d.ladder_norm, 0);
  let r, g, b;
  if (n <= 0.5) {
    const t = n * 2;
    r = Math.round(184 + (210 - 184) * t);
  ...
  radarChart.data.datasets[0].label = d.country || "Custom Profile";
...
function barDetail(cfg, d) {
  ...
  if (cfg.key === "corruption_norm") {
    ...
    return wet > 0.5 ? `Tremolo ${rate} Hz (unstable)` : wet > 0.1 ? `Tremolo ${rate} Hz (light)` : "Steady (clean)";
  }
...
async function initAudio() {
  ...
  // Integrity → tremolo (fast wobble = unstable/corrupt, silent = clean)
  tremoloNode = new Tone.Tremolo({ frequency: 8, depth: 0.8, wet: 0 }).start();

  // Freedom → auto-panning (rate and wet vary per country)
  pannerNode = new Tone.AutoPanner({ frequency: 1, wet: 0 }).start();

  // Lower master volume to compensate for 3-note chords
  ...
  // Signal chain: polySynth → filter → tremolo → panner → reverb → volume → output
  polySynth.chain(filterNode, tremoloNode, pannerNode, reverb, volNode, Tone.getDestination());

  audioReady = true;
}

// Build a chord (array of note strings) from a root note and semitone intervals
...
function buildSoundParams(d) {
  ...
  // ── Happiness → chord voicing ──
  // Base root-note melody (8 steps)
  let baseNotes;
  let chordIntervals;
  if (ladderN > 0.75) {
    // Lydian / bright: open fifth + octave
  ...
  // ── Integrity → tritone substitution + distortion ──
  // Low corruptionN = low integrity = audible corruption in the melody
  const processedNotes = [...baseNotes];
  if (corruptionN < 0.35) {
    processedNotes[2] = Tone.Frequency(baseNotes[0]).transpose(6).toNote();
  }

  // ── Health → octave register ──
  const octaveShift = healthN > 0.67 ? 12 : healthN < 0.33 ? -12 : 0;

  // Build chord array for each step
  const melodyChords = processedNotes.map(note => {
    const shifted = Tone.Frequency(note).transpose(octaveShift).toNote();
    return buildChord(shifted, chordIntervals);
  });

  // ── Generosity → rhythm density (2–8 active steps per 8-step pattern) ──
  const numActive   = Math.max(2, Math.round(2 + generosityN * 6));
  // Fill priority: beats 1, 5, 3, 7, 2, 4, 6, 8 (downbeats first, then upbeats)
  const fillOrder   = [0, 4, 2, 6, 1, 3, 5, 7];
  const pattern     = new Array(8).fill(null);
  for (let i = 0; i < numActive; i++) {
    pattern[fillOrder[i]] = melodyChords[fillOrder[i]];
  }

  // ── Freedom → stereo panning rate & depth ──
  const pannerRate = 0.3 + freedomN * 2.0;   // 0.3 – 2.3 Hz
  const pannerWet  = freedomN * 0.85;         // 0 → 0.85

  // ── Integrity → tremolo ──
  // Low corruptionN = low integrity = fast, deep wobble
  // Threshold at 0.5: above that, signal is clean (wet = 0)
  const tremoloWet  = corruptionN < 0.5 ? (1 - corruptionN * 2) * 0.9 : 0;
  const tremoloRate = 3 + (1 - corruptionN) * 9;  // 3–12 Hz (slow throb → fast flutter)

  return {
    pattern,
    bpm:           Math.round(60 + gdpN * 78),
    noteDur:       "4n",             // sustained chords; 4-beat at 8n subdivision
    velocity:      0.55,             // fixed (generosity now drives density, not volume)
...
async function playSoundForCountry(d, label) {
  ...
  // Apply all per-country parameters
  ...
  // Sequence over 8-step pattern; null entries are silent rests
...
function updatePgRadarChart(vals) {
  const n = safe(vals.ladder_norm, 0.5);
  let r, g, b;
  if (n <= 0.5) {
    const t = n * 2;
    r = Math.round(184 + (210 - 184) * t);
  ...
  pgRadarChart.data.datasets[0].borderColor         = color;
  pgRadarChart.data.datasets[0].pointBackgroundColor = color;
...
function updatePlayground() {
  ...
  // Refresh value labels + fill gradient
  PG_SLIDERS.forEach(s => {
  ...
  // Keep playground radar in sync with sliders
  ...
  document.getElementById("pg-ladder").textContent      = fmtVal(match.ladder);
  document.getElementById("pg-gdp").textContent         = fmtVal(match.gdp);
  document.getElementById("pg-health").textContent      = fmtVal(match.health);
  document.getElementById("pg-freedom").textContent     = fmtVal(match.freedom);
  document.getElementById("pg-generosity").textContent  = fmtVal(match.generosity);
  document.getElementById("pg-corruption").textContent  = fmtVal(match.corruption);

  // Auto-update sound if playing (debounced)
  if (pgPlaying) {
...
function initPlayground() {
  ...
  // Attach slider listeners + init fill gradients
  PG_SLIDERS.forEach(s => {
    const slider = document.getElementById(s.id);
    updateSliderFill(slider);
  ...
  document.getElementById("pgPlayBtn").addEventListener("click", async () => {
    ...
    try {
      ...
      console.error("[Playground] play error:", err);
    }
  });
  ...
  // Initial render — must run after all listeners are attached above
  updatePlayground();
}
